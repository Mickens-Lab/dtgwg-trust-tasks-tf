...
summary: A participating party submits its presentation, bound to the session challenge; the witness's mandatory response delivers the Verifiable Witness Credential. This response is the outcome evidence a VWC presentation must ship — the terminal document a taskContext points at.
status: draft
...
Within an open witness ceremony
([`witness/session`](../../0.1/spec.md)), a participating party submits its
**presentation**, bound to the session's `{challenge, domain}`. The witness
verifies it and — in the **mandatory** `#response` — delivers the
**Verifiable Witness Credential** attesting the witnessed exchange.

## This response is the outcome evidence

The `#response` of this specification is the ceremony's **terminal success
...
1. The delivered VWC's `taskContext` **MUST** equal the `id` of the
   `witness/session` document that opened this ceremony — the innermost
...
2. A holder later presenting that VWC as proof the witnessing occurred
   **MUST** retain this `#response` and ship it with the presentation. A
   ...
   `#response`; and the evidence's own REQUIRED proof verifies.
...
1. Emits a document whose `type` is `https://trusttasks.org/spec/witness/session/submit/0.1`, on the ceremony's thread (`threadId` = the `witness/session` document's `id`).
...
3. On success, **MUST** return the `#response` delivering the VWC, under the REQUIRED proof, with the `taskContext` rule of this document honoured.
...
proves is that the ceremony reached its terminal success state. Neither
substitutes for the other: a valid VWC with no paired evidence proves a
witness once signed something, not that this ceremony completed.
...
witness.** That is the ceremony's purpose, and the reason `exposure`
